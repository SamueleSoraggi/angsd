...
void abcDstat2::printArg(FILE *argFile){
  ...
  fprintf(argFile,"\t-enhance\t\t\t%d\tonly analyze sites where H4 is non poly",enhance);
  fprintf(argFile,"\n");
}
...
void abcDstat2::run(funkyPars *pars){
  ...
  if(doAbbababa2==1){
	...
	if(ABCD[s][i*4]<0 || ABCD[s][i*4+1]<0 || ABCD[s][i*4+2]<0 || ABCD[s][i*4+3]<0)
	  contNeg += 1;                //count negative occurrences in site s
	if(ABCD[s][i*4]<maxDepth && ABCD[s][i*4+1]<maxDepth && ABCD[s][i*4+2]<maxDepth && ABCD[s][i*4+3]<maxDepth){
	  //if(ABCD[s][i*4] + ABCD[s][i*4+1] + ABCD[s][i*4+2] + ABCD[s][i*4+3] < maxDepth){
 ...
 // allele counts
	...
	for(int j=0;j<4;j++){
	  for(int k=0;k<4;k++){
		...
		//fprintf(stderr,"%f,%f,%f,%f--",ABCD2[i] , ABCD2[4+j] , ABCD2[8+k] , ABCD2[12+l]);
		//fprintf(stderr,"%d%d%d%d-%f\t",i,j,k,l,ALLELES[posiz]);	      
	      posiz++;
	    }
	  }
	}
